 ...
 *
 * PASSWORD (v1.7)
 *   The manager password now lives in a CELL in the PRIVATE membership Config
 *   sheet — same place the coaches app keeps its passcode. Look on the "Config"
 *   tab, SETTINGS section, key "photo_passcode". To change the password, just
 *   edit that cell. No more hidden Script Property.
 *   (The old ADMIN_KEY Script Property is no longer used — you can delete it from
 *    Project Settings → Script Properties whenever you like.)
 *
 * INSTALL
 *   1. Go to script.google.com (signed in as SDTSAdmin) → open this project.
 *   2. Paste this file in (replace all). Save.
 *   3. Deploy → Manage deployments → Edit → New version → Deploy.
 *      (First run may re-prompt for Drive + Sheets permissions — approve them.)
 *   4. Open the web-app URL, enter the password from the Config cell.
...
var CONFIG_SHEET_ID = '1Xw8ZtnVGdbjjEkQ4AYLCv7q-8owpdyVmmblo6fimkAo';      // SDTS Site Config sheet (PUBLIC — Photos tab only)
var PHOTOS_TAB      = 'Photos';
var ARCHIVE_NAME    = 'Archive';                                          // subfolder for removed photos

// Password source — PRIVATE membership Config sheet (never the public site config above).
var MEMBERSHIP_CONFIG_SHEET_ID = '1eKgwPS1dRvpXQGY8TnnxhcNrvWuS4tSf9TBQNhjktXM'; // private membership Config sheet
var MEMBERSHIP_CONFIG_TAB      = 'Config';
var PHOTO_PASSCODE_KEY         = 'photo_passcode';                         // the key in column A whose value (col B) is the password
...
/* ---------------- auth ---------------- */
// Read the password from the membership Config sheet (Config tab, key "photo_passcode").
// Mirrors how the coaches app reads coach_passcode — a visible cell, not a hidden setting.
function photoPasscode_() {
  var ss = SpreadsheetApp.openById(MEMBERSHIP_CONFIG_SHEET_ID);
  var sh = ss.getSheetByName(MEMBERSHIP_CONFIG_TAB);
  if (!sh) throw new Error('Config tab "' + MEMBERSHIP_CONFIG_TAB + '" not found in the membership Config sheet.');
  var n = sh.getLastRow();
  if (n < 1) return '';
  var vals = sh.getRange(1, 1, n, 2).getValues();   // col A = key, col B = value
  for (var i = 0; i < vals.length; i++) {
    if (String(vals[i][0]).trim() === PHOTO_PASSCODE_KEY) return String(vals[i][1]).trim();
  }
  return '';
}

function checkKey_(key) {
  var want = photoPasscode_();
  if (!want) throw new Error('No "photo_passcode" is set in the membership Config sheet (Config tab, SETTINGS section).');
  if (String(key).trim() !== String(want)) throw new Error('Wrong password.');
  return true;
}
